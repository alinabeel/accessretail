...
""" ------------------------- RBD Summary ------------------------- """
class RBDSummaryAJAX(LoginRequiredMixin, generic.View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            #Cell Query List
            queryList = RBD.objects.all().filter(country = country).exclude(parent_id = None).order_by('tree_id')
            ...
            for k in range(0,len(queryList)):
                rbd_id = queryList[k].id
                rbd_parent_id = queryList[k].parent_id
                cdebug(rbd_id)
                cdebug(rbd_parent_id)
                queryListRBDParent = RBD.objects.get(country = country, id = rbd_parent_id)

                rbd_serialize_str = queryList[k].serialize_str
                rbd_parent_serialize_str = queryListRBDParent.serialize_str

                cdebug(rbd_parent_serialize_str)

                print(Colors.BOLD_YELLOW,'Processing RBD: ', queryListRBDParent.name,Colors.WHITE)
                print(Colors.BOLD_YELLOW,'Processing Sub-RBD: ', queryList[k].name,Colors.WHITE)
                ...
                print(Colors.GREEN)
                if rbd_parent_serialize_str != '':
                    rbd_parent_params = parse_qs((rbd_parent_serialize_str))
                    rbd_parent_list = getDictArray(rbd_parent_params,'field_group[group]')
                    rbd_parent_dic = getDicGroupList(rbd_parent_list)
                    rbd_parent_group_filter = getGroupFilter(rbd_parent_dic)
                    rbd_parent_group_filter_human = getGroupFilterHuman(rbd_parent_dic)


                field_group = parse_qs((rbd_serialize_str))
                ...
                filter_human = ''
                if rbd_parent_serialize_str != '':
                    rbd_parent_group_filter &= Q(group_filter)
                    filter_human = "RBD(\n{}) \n AND \n CELL( \n {})".format(rbd_parent_group_filter_human, group_filter_human)
                    queryListPPCell = queryListPPCell.filter(rbd_parent_group_filter)
                else:
                    if(group_filter != ''):
                ...
                #Get outlet ids from Cell panel profile previous month
                ...
                total_outlets_in_audit_current = agg_outlets_audit_all_current['count']

                """
                    Unprojected Sales (Volume)	Unprojected Sales (Value)	projected Sales (Volume)	projected Sales (Value)
                """

                #Get outlet ids from panel profile current month
                ...
                temp_dic = {
                    'RBD Name' : queryListRBDParent.name,
                    'SUB-RBD Name' : queryList[k].name,
                    'SUB-RBD Code' : queryList[k].code,
                    'SUB-RBD Description' : queryList[k].description,
        ...
        if export is not False:
            ...
            response['Content-Disposition'] = 'attachment; filename=rbd_summary.csv'
...
class RBDSummaryListView(LoginRequiredMixin, generic.TemplateView):
    template_name = "reports/rbd_summary.html"
    PAGE_TITLE = "RBD Summary"
    extra_context = {
        'page_title': PAGE_TITLE,
        'header_title': PAGE_TITLE
    }
...
class RBDSummaryOverviewAJAX(LoginRequiredMixin, generic.View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            for k in range(0,len(queryRBDList)):
                ...
                cell_dic = {}
                queryCellList = RBD.objects.all().filter(country = country,parent_id=rbd_id)
...
class RBDSummaryOverviewListView(LoginRequiredMixin, generic.TemplateView):
    template_name = "reports/rbd_summary_overview.html"
    PAGE_TITLE = "RBD Summary Overview"
    extra_context = {
        'page_title': PAGE_TITLE,
        'header_title': PAGE_TITLE
    }
...
class RBDShopInspectionAJAX(LoginRequiredMixin, generic.View):
    ...
    def get(self, request, *args, **kwargs):
            ...
            for k in range(0,len(queryList)):
                ...
                for o in range(0,len(outlets)):
                    ...
                    if(is_usable_previous):
                        ...
                        # for i in range(0,len(queryListPPCellPrevious)):
                        # prettyprint_queryset(queryListPPCellPrevious)
                        agg_outlets_cell_previous = queryListPPCellPrevious.aggregate(count = Count('id'),
                                                                                    num_factor_avg = Avg('num_factor'),
                                                                                    turnover_sum = Sum('turnover'))
        ...
        if export is not False:
            response = HttpResponse(content_type='text/csv')
            response['Content-Disposition'] = 'attachment; filename=rbd_shop_inspection.csv'
            csv_writer = csv.writer(response)
            count = 0
            for d in return_dic['results']:
                if count == 0:
...
class RBDShopInspectionListView(LoginRequiredMixin, generic.TemplateView):
    template_name = "reports/rbd_shop_inspection.html"
    PAGE_TITLE = "RBD and Shop Inspection"
    extra_context = {
        'page_title': PAGE_TITLE,
        'header_title': PAGE_TITLE
    }
